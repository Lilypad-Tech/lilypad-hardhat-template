...
if [ ! -d $LILYPAD_DIR ]; then
    ...
    #make
    #cd ../../..
    #chmod +x $LILYPAD_DIR/code/lilypad/bin/lilypad
    #mv $LILYPAD_DIR/code/lilypad/bin/lilypad /usr/local/bin
    #rm -rf $LILYPAD_DIR/code

    LILYPAD_VERSION="v2.0.0-f957f9e"
    # LILYPAD_BINARY="lilypad-darwin-arm64"
    LILYPAD_BINARY="lilypad-linux-amd64"

    #sudo apt update
    #sudo apt install libc6 -y
    # wget https://github.com/Lilypad-Tech/lilypad/releases/download/v2.0.0-f957f9e/lilypad-linux-amd64
    wget https://github.com/Lilypad-Tech/lilypad/releases/download/$LILYPAD_VERSION/$LILYPAD_BINARY
    chmod +x $LILYPAD_BINARY
    mv $LILYPAD_BINARY /usr/local/bin/lilypad
    rm $LILYPAD_BINARY
